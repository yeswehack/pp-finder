...
dist/loader.cjs: ${SOURCES}
	...
	@mkdir -p dist
	@TMP_DIR="$$(mktemp -d)" && \
	yarn run ncc build -q -t -o "$${TMP_DIR}" "./src/loader.ts" && \
...
dist/cli.cjs: ${SOURCES}
	...
	@mkdir -p dist
	@TMP_DIR="$$(mktemp -d)" && \
	yarn run ncc build -q -t -o "$${TMP_DIR}" "./src/cli.ts" && \
...
dist/compiler.js: ${SOURCES}
	...
	@mkdir -p dist
	@TMP_DIR="$$(mktemp -d)" && \
	yarn run ncc build -q -t -o "$${TMP_DIR}" "./src/compiler.ts" && \
...
dist/pp-finder.schema.json: ./src/config.ts
	...
	@mkdir -p dist
	@yarn run ts-node > dist/pp-finder.schema.json <<< "import { zodToJsonSchema } from 'zod-to-json-schema';\
